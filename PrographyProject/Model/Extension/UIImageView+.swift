...
extension UIImageView {
    func setThumbnailImageFromCache(_ thumbnailImageURLString: String, placeHolder _: UIImage?) {
    ...
    func setThumbnailImageFromServerURL(_ thumbnailImageURLString: String, placeHolder: UIImage?) {
        // 1) 재사용셀의 잘못된 이미지설정을 방지하기 위해 이미지 셋팅 전 이미지뷰는 nil값 처리를 한다.
        image = nil

        // 2) 인자값으로 받은 URL에 대한 이미지가 이미 있다면,
        if let cachedImage = MovieCommonData.shared.thumbnailImageCache.object(forKey: NSString(string: thumbnailImageURLString)) {
            // 이미지셋팅 후 작업을 마친다. 이미지캐시가 없다면 URL에 대한 데이터처리를 준비한다.
            image = cachedImage
            return
        ...
        // 3) 인자값으로 받은 URL에 URLSession을 통해 이미지를 요청한다.
        if let imageUrl = URL(string: thumbnailImageURLString) {
            URLSession.shared.dataTask(with: imageUrl, completionHandler: { data, _, error in
                ...
                if error != nil {
                    print("thumbnailImage URL Loading Error!")
                    DispatchQueue.main.async {
                        self.image = placeHolder // 이미지의 설정은 비동기 과정에서 진행한다.
                    }
                    return
                }
